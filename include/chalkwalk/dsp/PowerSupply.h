...
// AN ANALOGUE MACHINE'S POWER SUPPLY.
//
// ---- WHY IT IS HERE AND NOT IN chalkwalk-tape ----
//
// It arrived there with the tape physics and did not belong: nothing about a
// reservoir capacitor is magnetic. What made it look like tape is its
// PROVENANCE -- the figures and the diagnostic below come from tape-machine
// service literature, because that is the corpus this ecosystem had read --
// and provenance is not the same as scope. A console, a preamp and a synth
// all run out of rail the same way.
//
// The citations are to Remanence, the project that derived this
// (`DESIGN.md` §4.11, `SOURCES §21`), and they stay: a number whose source
// cannot be named is not a number worth keeping.
//
