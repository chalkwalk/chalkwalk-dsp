...
//
// Nothing here has a dependency. Loudness used to, and lived here, which meant
// every consumer of `peak` compiled `<ebur128.h>` whether it wanted BS.1770 or
// not; it is `Loudness.h` now, same namespace, and only that file needs the
// `chalkwalk::dsp::measure` target. Everything in THIS file is arithmetic over
// a buffer and needs no more than `chalkwalk::dsp`.
//
