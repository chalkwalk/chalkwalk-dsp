// Minimum-phase design from a magnitude (chalkwalk/dsp/MinimumPhase.h, SOURCES 15).
//
          ...
          "[minphase][teeth]")
{
    ...
    const auto h = dsp::designMinimumPhase(48, fs, magnitude);
    REQUIRE(h.size() == 48);
          ...
          "[minphase][teeth]")
{
    ...
    const int taps = 128;
    const auto minimum = dsp::designMinimumPhase(taps, fs, magnitude);
    ...
    const double minimumDelay = dsp::medianEnergyDelay(minimum);
    const double symmetricDelay = dsp::medianEnergyDelay(symmetric);
          ...
          "[minphase][teeth]")
{
    ...
    const auto tight = dsp::designMinimumPhase(64, fs, magnitude, 64);
    const auto roomy = dsp::designMinimumPhase(64, fs, magnitude);
...
TEST_CASE("the energy median is where the energy is", "[minphase]")
{
    ...
    REQUIRE(dsp::medianEnergyDelay(h) == Approx(exact));

    // An impulse is at zero, and silence has no answer rather than a wrong one.
    REQUIRE(dsp::medianEnergyDelay({ 1.0, 0.0, 0.0 }) == Approx(0.0));
    REQUIRE(dsp::medianEnergyDelay({ 0.0, 0.0 }) == Approx(0.0));
}
